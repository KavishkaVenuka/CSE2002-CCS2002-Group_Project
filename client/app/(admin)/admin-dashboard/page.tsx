  ...
  return (
    <div className="flex min-h-screen bg-nb-bg w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 bg-nb-white border-2 border-nb-black p-2 shadow-nb">
        ...
        >
          <RefreshCw className={`w-5 h-5 ${isLoading ? 'animate-spin' : ''}`} strokeWidth={2.5} />
          REFRESH DATA
        </button>
      </div>

      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-[60vh] bg-nb-yellow border-4 border-nb-black shadow-nb">
          <Loader2 className="w-16 h-16 animate-spin text-nb-black mb-4" />
          <p className="text-nb-black font-black uppercase tracking-widest text-xl">Aggregating real-time data...</p>
        </div>
      ) : (
        <>
          {/* KPI Cards */}
